...
VolumeInterior = (AreaFloor * HeightWall) + ((1/3) * AreaFloor * 3);     %m^3
...
R_F = Rcarpet + Rconcrete;  %100 percent of the time
...
for i = 1:length(t)-1
    ...
    Tint(i+1) = (((AreaWall * (Tair(i) - Tint(i)) / (R_W + (1/Hout) + (1/Hin))) + ...
        ...
        (AreaRoof * (Troof(i) - Tint(i)) / (R_R + (1/Hin))) + ...
    ...
    Troof(i+1) = ((((AreaRoof * (Tair(i) - Troof(i)) / (1/Hout)) + ...
        ...
        (AreaRoof * (Troof(i) - Tint(i)) / (R_R + (1/Hin)))) / ...
        (rho_roof * AreaRoof * t_roof * Cv_roof))* dt) + Troof(i);
end
...
% Qir = AreaRoof * (Troof(i) - Tint(i)) / (R_R + (1/Hin)); 
...
annualCostFuel_nominal = C_fuel(length(t));

%%Discounting
r = .05;
N = 30;

deltaC = annualCostFuel_nominal;
