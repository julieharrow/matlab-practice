...
figure(1)
b_0 = 0.1; % mean displacement to account for buoyancy
z_inv = 30; %m
...
for t=1:T
    ...
    omegaZ = sqrt(3)*(b_0 + unifrnd(-1,1,1,t*emitted));
...
title('Scatter plot of plume emitted from road for 2 hours without Z_inv')
...
for iy = 1:ngrid
    for iz = 1:ngrid
        ccontour(iz,iy) = sum((y>=gridy(iy)) & (y<=gridy(iy)+1)&...
